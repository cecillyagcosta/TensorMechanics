...
def detect_clouds(image, plot=False):
    """
    Detecta nuvens em uma imagem usando limiarização e segmentação.
    Se plot=True, exibe a imagem com as regiões detectadas.
    """
...
def calculate_cop(image):
    ...
    """
    # 1. Detectar nuvens
    cloud_centers, cloud_radii = detect_clouds(image, plot=True)  # Ative a visualização aqui
